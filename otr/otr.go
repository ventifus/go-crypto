...
const (
	...
	// conversation.
	ConversationEnded
	// Acceptable protocol version range
	minVersion = 2
	maxVersion = 3
)
...
var (
	...
	v2FragmentPrefix      = []byte("?OTR,")
	v3FragmentPrefix      = []byte("?OTR|")
	msgPrefix             = []byte("?OTR:")
	queryMarker           = []byte("?OTR")
)

func acceptedVersion(ver uint16) bool {
	return ver >= minVersion && ver <= maxVersion
}
...
func isQuery(msg []byte) (greatestCommonVersion uint16) {
	...
	for i, c := range msg[pos+len(queryMarker):] {
		...
		v := uint16(c - '0')
		if v > greatestCommonVersion && acceptedVersion(v) {
			greatestCommonVersion = v
		}
	}
...
func (c *Conversation) randTag() (r uint32) {
	var buf = make([]byte, 4)
	for r < 100 {
		_, err := io.ReadFull(c.rand(), buf)
		if err != nil {
...
func (c *Conversation) Receive(in []byte) (out []byte, encrypted bool, change SecurityChange, toSend [][]byte, err error) {
	if bytes.HasPrefix(in, v2FragmentPrefix) || bytes.HasPrefix(in, v3FragmentPrefix) {
	...
	// The first two bytes are the protocol version (2 or 3)
	if len(msg) < 3 {
		err = errors.New("otr: invalid OTR message")
		return
	}
	version := uint16(msg[1])
	if msg[0] != 0 || !acceptedVersion(version) {
	...
	if version == 3 {
		// Next 8 bytes are the instance tags
		if len(msg) < 8 {
			err = instanceError
			return
		}
