...
var idPKIXOCSPBasic = encoding_asn1.ObjectIdentifier([]int{1, 3, 6, 1, 5, 5, 7, 48, 1, 1})
...
func (c *certID) unmarshal(b *cryptobyte.String) bool {
	...
	if !b.ReadASN1(&cert, asn1.SEQUENCE) ||
		!readAlgorithmIdentifier(&cert, &c.HashAlgorithm) ||
		!cert.ReadASN1((*cryptobyte.String)(&c.NameHash), asn1.OCTET_STRING) ||
		!cert.ReadASN1((*cryptobyte.String)(&c.IssuerKeyHash), asn1.OCTET_STRING) ||
...
func (c *certID) Marshal(b *cryptobyte.Builder) error {
	...
	b.AddASN1(asn1.SEQUENCE, func(cert *cryptobyte.Builder) {
		cert.MarshalASN1(c.HashAlgorithm)
		cert.AddASN1(asn1.OCTET_STRING, func(nameHash *cryptobyte.Builder) {
			nameHash.AddBytes(c.NameHash)
		})
		cert.AddASN1(asn1.OCTET_STRING, func(keyHash *cryptobyte.Builder) {
...
func (r *ocspRequest) unmarshal(b *cryptobyte.String) bool {
	...
	if !b.ReadASN1(&ocsp, asn1.SEQUENCE) ||
		!r.TBSRequest.unmarshal(&ocsp) ||
		!ocsp.ReadOptionalASN1(&sigIgnored, nil /* present */, asn1.Tag(0).ContextSpecific().Constructed()) ||
		!ocsp.Empty() {
		return false
	}
	return true
...
func (r *ocspRequest) Marshal(b *cryptobyte.Builder) error {
	b.AddASN1(asn1.SEQUENCE, func(ocsp *cryptobyte.Builder) {
...
//     requestList                 SEQUENCE OF Request,  /* exactly one must be given */
...
type tbsRequest struct {
	Version       int
	RequestorName pkix.RDNSequence
	// Exactly one request must be given, so we flatten the Request SEQUENCE.
	Cert certID
}
...
func (r *tbsRequest) unmarshal(b *cryptobyte.String) bool {
	...
	if !b.ReadASN1(&tbs, asn1.SEQUENCE) ||
		!tbs.ReadOptionalASN1Integer(&r.Version, asn1.Tag(0).ContextSpecific().Constructed(), 0) ||
		!tbs.ReadOptionalASN1(&requestorName, &reqNamePresent, asn1.Tag(1).ContextSpecific().Constructed()) ||
		!tbs.ReadASN1(&requestList, asn1.SEQUENCE) ||
		!requestList.Empty() && (!r.unmarshalRequestList(&requestList) ||
			!requestList.Empty()) ||
		!tbs.ReadOptionalASN1(&ignoredExtensions, nil /* present */, asn1.Tag(2).ContextSpecific().Constructed()) ||
		!tbs.Empty() {
		return false
	}
	if reqNamePresent {
		// TODO(martinkr): Make pkix use cryptobyte.
		if rest, err := encoding_asn1.Unmarshal(requestorName, &r.RequestorName); err != nil || len(rest) > 0 {
			return false
		}
	}
	return true
...
func (r *tbsRequest) unmarshalRequestList(s *cryptobyte.String) bool {
	// requestList must have exactly one Request
	var reqList, extIgnored cryptobyte.String
	if !s.ReadASN1(&reqList, asn1.SEQUENCE) ||
		!r.Cert.unmarshal(&reqList) ||
		!reqList.ReadOptionalASN1(&extIgnored, nil /* present */, asn1.Tag(0).ContextSpecific().Constructed()) ||
		!reqList.Empty() {
		return false
	}
	return true
...
func (r *tbsRequest) Marshal(b *cryptobyte.Builder) error {
	b.AddASN1(asn1.SEQUENCE, func(tbs *cryptobyte.Builder) {
		if r.Version != 0 {
			tbs.AddASN1(asn1.Tag(0).ContextSpecific().Constructed(), func(version *cryptobyte.Builder) {
				version.AddASN1Int64(int64(r.Version))
			})
		}
		if len(r.RequestorName) > 0 {
			tbs.AddASN1(asn1.Tag(1).ContextSpecific().Constructed(), func(requestor *cryptobyte.Builder) {
				requestor.MarshalASN1(&r.RequestorName)
			})
		}
		// requestList contains exactly one value.
		tbs.AddASN1(asn1.SEQUENCE, func(requestList *cryptobyte.Builder) {
			requestList.AddASN1(asn1.SEQUENCE, func(req *cryptobyte.Builder) {
				req.AddValue(&r.Cert)
			})
		})
	})
...
func (r *responseASN1) unmarshal(b *cryptobyte.String) bool {
	...
	if !b.ReadASN1(&ocsp, asn1.SEQUENCE) ||
		...
		!ocsp.ReadOptionalASN1(&respBytes, &respPresent, asn1.Tag(0).ContextSpecific().Constructed()) ||
...
func (r *responseASN1) Marshal(b *cryptobyte.Builder) error {
	b.AddASN1(asn1.SEQUENCE, func(ocsp *cryptobyte.Builder) {
		...
		if len(r.Response.Response) > 0 {
			ocsp.AddASN1(asn1.Tag(0).ContextSpecific().Constructed(), func(resp *cryptobyte.Builder) {
...
type responseBytes struct {
	ResponseType encoding_asn1.ObjectIdentifier
	Response     []byte
}

func (r *responseBytes) unmarshal(b *cryptobyte.String) bool {
	var resp cryptobyte.String
	if !b.ReadASN1(&resp, asn1.SEQUENCE) ||
		!resp.ReadASN1ObjectIdentifier(&r.ResponseType) ||
		!resp.ReadASN1((*cryptobyte.String)(&r.Response), asn1.OCTET_STRING) ||
		!resp.Empty() {
		return false
	}
	return true
...
func (r *responseBytes) Marshal(b *cryptobyte.Builder) error {
	b.AddASN1(asn1.SEQUENCE, func(resp *cryptobyte.Builder) {
...
type basicResponse struct {
	...
	Signature          encoding_asn1.BitString
	Certificates       []encoding_asn1.RawValue
}
...
func (r *basicResponse) unmarshal(b *cryptobyte.String) bool {
	...
	if !b.ReadASN1(&resp, asn1.SEQUENCE) ||
		...
		!resp.ReadOptionalASN1(&certs, &certsPresent, asn1.Tag(0).ContextSpecific().Constructed()) ||
		!resp.Empty() {
		return false
	}
	if certsPresent {
		// TODO(martinkr): Add ReadASN1RawValue?
		if rest, err := encoding_asn1.Unmarshal(certs, &r.Certificates); err != nil || len(rest) > 0 {
			return false
		}
	}
	return true
...
func (r *basicResponse) Marshal(b *cryptobyte.Builder) error {
	b.AddASN1(asn1.SEQUENCE, func(resp *cryptobyte.Builder) {
		...
		resp.AddASN1BitString(r.Signature.Bytes)
		if len(r.Certificates) > 0 {
			resp.AddASN1(asn1.Tag(0).ContextSpecific().Constructed(), func(certs *cryptobyte.Builder) {
...
func (r *responseData) unmarshal(b *cryptobyte.String) bool {
	...
	if !b.ReadASN1Element(&raw, asn1.SEQUENCE) {
		return false
	}
	r.Raw = raw
	if !raw.ReadASN1(&resp, asn1.SEQUENCE) || !raw.Empty() ||
		!resp.ReadOptionalASN1Integer(&r.Version, asn1.Tag(0).ContextSpecific().Constructed(), 0) ||
		!resp.ReadAnyASN1Element(&responderID, nil /* tag */) ||
		!r.unmarshalResponderID(&responderID) ||
		!responderID.Empty() ||
		!resp.ReadASN1GeneralizedTime(&r.ProducedAt) ||
		!resp.ReadASN1(&responses, asn1.SEQUENCE) ||
		!resp.ReadOptionalASN1(&extIgnored, nil, asn1.Tag(1).ContextSpecific().Constructed()) ||
		!resp.Empty() {
		return false
	}
	for !responses.Empty() {
...
func (r *responseData) unmarshalResponderID(s *cryptobyte.String) bool {
	var (
	...
	switch tag {
	case asn1.Tag(1).ContextSpecific().Constructed(): // Name
		var rdn pkix.RDNSequence
		// TODO(martinkr): Make pkix use cryptobyte.
		if rest, err := encoding_asn1.Unmarshal(content, &rdn); err != nil || len(rest) != 0 {
			return false
		}
		r.RawResponderName = content
	case asn1.Tag(2).ContextSpecific().Constructed(): // KeyHash
		if !content.ReadASN1Bytes(&r.ResponderKeyHash, asn1.OCTET_STRING) ||
			!content.Empty() {
			return false
		}
	default:
...
func (r *responseData) Marshal(b *cryptobyte.Builder) error {
	...
	b.AddASN1(asn1.SEQUENCE, func(resp *cryptobyte.Builder) {
		if r.Version != 0 {
			resp.AddASN1(asn1.Tag(0).ContextSpecific().Constructed(), func(version *cryptobyte.Builder) {
				version.AddASN1Int64(int64(r.Version))
			})
		}
		if (r.RawResponderName == nil) == (r.ResponderKeyHash == nil) {
			err = errors.New("exactly one of RawReponderName and ResponderKeyHash must be set")
			return
		}
		if r.RawResponderName != nil {
			resp.AddASN1(asn1.Tag(1).ContextSpecific().Constructed(), func(name *cryptobyte.Builder) {
				name.AddBytes(r.RawResponderName)
			})
		} else {
			resp.AddASN1(asn1.Tag(2).ContextSpecific().Constructed(), func(keyHash *cryptobyte.Builder) {
				keyHash.AddASN1OctetString(r.ResponderKeyHash)
			})
		}
		resp.AddASN1GeneralizedTime(r.ProducedAt)
		resp.AddASN1(asn1.SEQUENCE, func(responses *cryptobyte.Builder) {
...
type singleResponse struct {
	CertID           certID
	Good             encoding_asn1.Flag
	Revoked          revokedInfo
	Unknown          encoding_asn1.Flag
...
func (r *singleResponse) unmarshal(b *cryptobyte.String) bool {
	...
	if !b.ReadASN1(&resp, asn1.SEQUENCE) ||
		!resp.ReadASN1Element(&certID, asn1.SEQUENCE) ||
		!r.CertID.unmarshal(&certID) ||
		!certID.Empty() ||
		!r.unmarshalCertStatusChoice(&resp) ||
		!resp.ReadASN1GeneralizedTime(&r.ThisUpdate) ||
		!resp.ReadOptionalASN1(&nextUpdate, &nextUpdatePresent, asn1.Tag(0).ContextSpecific().Constructed()) ||
		(nextUpdatePresent && !nextUpdate.ReadASN1GeneralizedTime(&r.NextUpdate)) ||
		!nextUpdate.Empty() ||
		!resp.ReadOptionalASN1(&ext, &extPresent, asn1.Tag(1).ContextSpecific().Constructed()) ||
		!resp.Empty() {
		return false
	}
	if extPresent {
		// TODO(martinkr): Make pkix use cryptobyte.
		if rest, err := encoding_asn1.Unmarshal(ext, &r.SingleExtensions); err != nil || len(rest) > 0 {
			return false
		}
	}
	return true
...
func (r *singleResponse) Marshal(b *cryptobyte.Builder) error {
	b.AddASN1(asn1.SEQUENCE, func(resp *cryptobyte.Builder) {
		...
		// CertStatus CHOICE
		if r.Good {
			resp.AddBytes([]byte{byte(certStatusGood)})
		} else if (r.Revoked != revokedInfo{}) {
			resp.AddASN1(certStatusRevoked, func(revoked *cryptobyte.Builder) {
				revoked.AddValue(&r.Revoked) // N.B. Revoked.Marshal strips the tag.
			})
		} else if r.Unknown {
			resp.AddBytes([]byte{byte(certStatusUnknown)})
		}

		resp.AddASN1GeneralizedTime(r.ThisUpdate)
		if (r.NextUpdate != time.Time{}) {
			resp.AddASN1(asn1.Tag(0).ContextSpecific().Constructed(), func(nextUpdate *cryptobyte.Builder) {
				nextUpdate.AddASN1GeneralizedTime(r.NextUpdate)
			})
		}

		if len(r.SingleExtensions) > 0 {
			resp.AddASN1(asn1.Tag(1).ContextSpecific().Constructed(), func(ext *cryptobyte.Builder) {
...
func (r *revokedInfo) unmarshal(b *cryptobyte.String) bool {
	...
	if !b.ReadASN1GeneralizedTime(&r.RevocationTime) ||
		!b.ReadOptionalASN1(&reason, &reasonPresent, asn1.Tag(0).ContextSpecific().Constructed()) ||
		(reasonPresent && !reason.ReadASN1Enum(&r.Reason)) {
		return false
	}
	return true
}
...
func (r *revokedInfo) Marshal(b *cryptobyte.Builder) error {
	...
	if r.Reason != 0 {
		b.AddASN1(asn1.Tag(0).ContextSpecific().Constructed(), func(reason *cryptobyte.Builder) {
...
func signingParamsForPublicKey(pub interface{}, requestedSigAlgo x509.SignatureAlgorithm) (hashFunc crypto.Hash, sigAlgo pkix.AlgorithmIdentifier, err error) {
	...
	case *rsa.PublicKey:
		...
		sigAlgo.Parameters = encoding_asn1.RawValue{
			Tag: 5,
		}
		...
		tbsRequest{
			...
			Cert: certID{
				pkix.AlgorithmIdentifier{
					Algorithm:  hashAlg,
					Parameters: encoding_asn1.RawValue{Tag: 5 /* ASN.1 NULL */},
				},
				req.IssuerNameHash,
				req.IssuerKeyHash,
				req.SerialNumber,
...
func ParseRequest(bytes []byte) (*Request, error) {
	...
	hashFunc := getHashAlgorithmFromOID(req.TBSRequest.Cert.HashAlgorithm.Algorithm)
	...
	return &Request{
		...
		IssuerNameHash: req.TBSRequest.Cert.NameHash,
		IssuerKeyHash:  req.TBSRequest.Cert.IssuerKeyHash,
		SerialNumber:   req.TBSRequest.Cert.SerialNumber,
	}, nil
}
...
func ParseResponseForCert(bytes []byte, cert, issuer *x509.Certificate) (*Response, error) {
	...
	ret := &Response{
		...
		RawResponderName:   basicResp.TBSResponseData.RawResponderName,
		ResponderKeyHash:   basicResp.TBSResponseData.ResponderKeyHash,
	}
...
func CreateResponse(issuer, responderCert *x509.Certificate, template Response, priv crypto.Signer) ([]byte, error) {
	...
	if template.Certificate != nil {
		response.Certificates = []encoding_asn1.RawValue{{FullBytes: template.Certificate.Raw}}
	}
...
func readAlgorithmIdentifier(b *cryptobyte.String, out *pkix.AlgorithmIdentifier) bool {
	...
	if !b.ReadASN1Element(&alg, asn1.SEQUENCE) {
		return false
	}
	// TODO(martinkr): Make pkix use cryptobyte.
	if rest, err := encoding_asn1.Unmarshal(alg, out); err != nil || len(rest) != 0 {
		return false
	}
	return true
}
