...
func (resp *Response) CheckSignatureFrom(issuer *x509.Certificate) error {
	if resp.RawResponderName == nil && resp.ResponderKeyHash == nil {
		return errors.New("uninitialized OSCP response")
	}
	if !bytes.Equal(resp.RawResponderName, issuer.RawSubject) && !bytes.Equal(resp.ResponderKeyHash, issuer.SubjectKeyId) {
		return errors.New("responder id mismatch")
	}
