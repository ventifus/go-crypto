...
func TestKeyIdEcc256_0xD01055FBCADD268E(t *testing.T) {
	r := readerFromHex(ecc256PrivHex)
	curve := elliptic.P256()
	priv, err := ecdsa.GenerateKey(curve, r)
	if err != nil {
		t.Error(err)
	}
	pub := NewECDSAPublicKey(time.Unix(1297309478, 0), &priv.PublicKey)

	if pub.KeyId != 0xD01055FBCADD268E {
		t.Errorf("Unexpected key ID: %x", pub.KeyId)
	}
}
...
// Source: https://sites.google.com/site/brainhub/pgpecckeys#TOC-ECC-NIST-P-256-key-no-passphrase
// gpg2 --list-packets --verbose ecc-nist-p-256-brainhub-private.key
// pad skey[2] with eight leading zeroes
// subtract one from result
const ecc256PrivHex = `0000000000000000a355916f8665eb99c1af48d9560b5c6889e5287bc75aa693aaae9bdb15e8b3fc`
