...
func symDecrypt(content, password []byte) ([]byte, error) {
	...
	var ret []byte
	var nbPromptCalls int

	byteReader = bytes.NewReader(content)
	messageDetails, err = ReadMessage(byteReader, nil,
		func(keys []Key, symmetric bool) ([]byte, error) {
			// https://go-review.googlesource.com/#/c/16865/
			// the prompt func should yield an error else openpgp.read
			// will run into an infinite loop
			nbPromptCalls++
			if nbPromptCalls > 3 {
				return nil, errors.InvalidArgumentError("bad passphrase")
			}
			return password, nil
		}, nil)
	if err != nil {
...
func TestSymmetricBadPassword(t *testing.T) {
	...
	encrypted, err := symEncrypt(make([]byte, 100000), password1)
	if err == nil {
