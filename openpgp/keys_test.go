...
func TestMissingCrossSignature(t *testing.T) {
	...
	if len(keys) != 0 {
		t.Errorf("Accepted key with missing cross signature, %d", len(keys))
	}
	if err == nil {
	...
	if !strings.Contains(string(structural), "Missing cross-signature for a signing subkey") {
...
func TestInvalidCrossSignature(t *testing.T) {
	...
	if len(keys) != 0 {
		t.Errorf("Accepted key with invalid cross signature, %d", len(keys))
	}
	if err == nil {
	...
	if !strings.Contains(string(structural), "subkey signature invalid") {
		t.Fatalf("Unexpected error: %s", err)
	}
}
...
func TestGoodCrossSignature(t *testing.T) {
	...
	if len(keys[0].Subkeys) != 1 {
		t.Errorf("Failed to accept good subkey", len(keys[0].Subkeys))
	}
}
