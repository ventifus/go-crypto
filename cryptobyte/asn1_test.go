...
var readASN1TestData = []readASN1Test{
	...
	{"high tag", []byte{0x1f, 0x81, 0x80, 0x01, 2, 1, 2},
		0xff /* actually 0x4001, but tag is uint8 */, false, nil},
}
	...
	for _, test := range readASN1TestData {
		t.Run(test.name, func(t *testing.T) {
			...
			if ok != test.ok || ok && !bytes.Equal(out, test.out.([]byte)) {
				t.Errorf("in.ReadASN1() = %v, want %v; out = %v, want %v",
					ok, test.ok, out, test.out)
			}
		})
	}
}
...
func TestReadASN1GeneralizedTime(t *testing.T) {
	...
	for i, test := range testData {
		var in String = append([]byte{asn1.TagGeneralizedTime, byte(len(test.in))}, test.in...)
		var out time.Time
...
func TestReadASN1BitString(t *testing.T) {
	testData := []struct {
		...
		{[]byte{0x07, 0x00}, true, asn1.BitString{[]byte{0}, 1}},
	...
	for i, test := range testData {
		var in String = append([]byte{3, byte(len(test.in))}, test.in...)
