...
func TestMontgomeryPublic(t *testing.T) {
	pub, err := PublicFromMontgomery(&montgomeryPublic)
	if err != nil {
		t.Fatal(err)
	}
	for i, p := range pub {
		if p != edPublic[i] {
			t.Fatal("Public key conversion does not match")
		}
	}
}
