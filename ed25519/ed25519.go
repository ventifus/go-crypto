...
// PublicFromMontgomery creates a pubic key from an equivalent key in montgomery
// format.
func PublicFromMontgomery(k *[32]byte) (publicKey PublicKey, err error) {
	var u, y edwards25519.FieldElement

	if edwards25519.ScIsReduced(k) == 0 {
		return nil, errors.New("ed25519: montgomery publickey is not reduced")
	}

	edwards25519.FeFromBytes(&u, k)
	edwards25519.FeMontXToEdY(&y, &u)
	var publicKeyBytes [PublicKeySize]byte
	edwards25519.FeToBytes(&publicKeyBytes, &y)
	publicKey = make([]byte, PublicKeySize)
