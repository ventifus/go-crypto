...
type NSSCert struct {
	...
	// DistrustAfter, if non-nil, is the date after which certificates which
	// chain to Certificate should be distrusted.
...
func parseTrustClass(s *bufio.Scanner) ([sha1.Size]byte, *trustObj, error) {
	...
	for s.Scan() {
		...
		if l == "CKA_CERT_SHA1_HASH MULTILINE_OCTAL" {
			...
			copy(h[:], hash)
		} else if l == "CKA_TRUST_SERVER_AUTH CK_TRUST CKT_NSS_TRUSTED_DELEGATOR" { // We only care about server auth
			to.trusted = true
		}
	}
...
func ParseNSSCertData(r io.Reader) ([]NSSCert, error) {
	...
	for _, e := range entries {
		...
		nssCert := NSSCert{Certificate: e.cert.c, DistrustAfter: e.cert.DistrustAfter}
