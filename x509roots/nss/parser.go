...
// rely on the golang.org/x/crypto/x509roots/fallbacks package which
...
// ParseCertData parses a NSS certdata.txt formatted file, returning only
...
//
// ParseCertData is not intended to be a general purpose parser for
// certdata.txt.
func ParseCertData(r io.Reader) ([]Cert, error) {
	...
	for _, e := range entries {
		...
		nssCert := Cert{Certificate: e.cert.c}
