...
func main() {
	...
	sort.Slice(certs, func(i, j int) bool {
		return certs[i].Certificate.Subject.String() < certs[j].Certificate.Subject.String()
	})

	b := bytes.NewBuffer(nil)
	b.Write([]byte(fmt.Sprintf(tmpl, time.Now().Format(time.RFC1123))))
	...
	for _, c := range certs {
		...
		b.Write([]byte(fmt.Sprintf("# %s\n# %x\n", c.Certificate.Subject.String(), sha256.Sum256(c.Certificate.Raw))))
		pem.Encode(b, &pem.Block{Type: "CERTIFICATE", Bytes: c.Certificate.Raw})
	}
	b.Write([]byte("`\n"))
