...
func pbDecrypterFor(algorithm pkix.AlgorithmIdentifier, password []byte) (cipher.BlockMode, int, error) {
	...
	case algorithm.Algorithm.Equal(oidPBES2):
		...
		return pbes2DecrypterFor(algorithm, utf8Password)
	default:
...
func pbes2DecrypterFor(algorithm pkix.AlgorithmIdentifier, password []byte) (cipher.BlockMode, int, error) {
	...
	if !params.Kdf.Algorithm.Equal(oidPBKDF2) {
		return nil, 0, NotImplementedError("kdf algorithm " + params.Kdf.Algorithm.String() + " is not supported")
	}
	...
	if kdfParams.Salt.Tag != asn1.TagOctetString {
		return nil, 0, errors.New("pkcs12: only octet string salts are supported for pbkdf2")
	}
	...
	case params.EncryptionScheme.Algorithm.Equal(oidAES256CBC):
		...
		block = b
	default:
		return nil, 0, NotImplementedError("pbes2 algorithm " + params.EncryptionScheme.Algorithm.String() + " is not supported")
	}
