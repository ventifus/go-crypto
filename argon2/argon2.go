...
// `key := argon2.Key([]byte("some password"), salt, 5, 64*1024, 4, 32)`
//
// The recommended parameters for interactive logins as of 2017 are time=4, memory=32*1024 and threads=1.
// Higher values, like time=6, memory=128*1024 and threads=1, increase security. The parameters time,
// memory, and threads should be increased as memory latency and CPU parallelism increases.
...
func processBlocks(B [][128]uint64, time, memory, threads uint32, mode int) {
	const syncPoints = 4
	var tmp, addresses, in, zero [128]uint64
	lanes := memory / threads
	segments := lanes / syncPoints
	for n := uint32(0); n < time; n++ {
		for slice := uint32(0); slice < syncPoints; slice++ {
			for lane := uint32(0); lane < threads; lane++ {
				if mode == argon2i || (mode == argon2id && n == 0 && slice < 2) {
					for i := range zero {
						zero[i] = 0
					}
					for i := range in {
				...
				if n == 0 && slice == 0 {
					...
					if mode == argon2i || (mode == argon2id && n == 0 && slice < 2) {
						nextAddresses(&addresses, &in, &zero) // compute 'random' addresses for first blocks
					}
				}
				...
				for index < segments {
					...
					if mode == argon2i || (mode == argon2id && n == 0 && slice < 2) {
						if index%128 == 0 {
							nextAddresses(&addresses, &in, &zero)
						}
					...
					processBlock(&B[offset], &tmp, &B[prev], &B[newOffset], true)
...
func nextAddresses(out, in, zero *[128]uint64) {
	in[6]++
	var tmp [128]uint64
	processBlock(out, &tmp, in, zero, false)
	processBlock(out, &tmp, out, zero, false)
}
