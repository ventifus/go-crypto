...
// func ladderStepLeg(w *[5]fp.Elt, move uint)
...
TEXT ·ladderStepLeg(SB),NOSPLIT,$192
...
//  (t0,t1) are two fp.Elt of fp.Size bytes, and
//  (b0,b1) are two fp.bigElt of 2*fp.Size bytes.
TEXT ·ladderStepBmi2Adx(SB),NOSPLIT,$192
    MOVQ work+0(FP), regWork
    MOVQ move+8(FP), regMove
    addSub(x2,z2)
    addSub(x3,z3)
...
// work contains variables used in the Joye's ladder step,
// where {x1,z1,x2,z2} are four fp.Elt of fp.Size bytes, and
//       {b0,b1} are two fp.bigElt of 2*fp.Size bytes.
TEXT ·difAddLeg(SB),NOSPLIT,$192
    MOVQ work+0(FP), regWork
    MOVQ mu+8(FP), regMu
...
// where {x1,z1,x2,z2} are four fp.Elt of fp.Size bytes, and
//       {b0,b1} are two fp.bigElt of 2*fp.Size bytes.
TEXT ·difAddBmi2Adx(SB),NOSPLIT,$192
    MOVQ work+0(FP), regWork
    MOVQ mu+8(FP), regMu
    MOVQ swap+16(FP), regSwap
    cswap(x1,x2,regSwap)
...
// work contains variables used in the Joye's ladder step,
// where {x1,z1,x2,z2} are four fp.Element of fp.Size bytes, and
//       {b0,b1} are two fp.bigElement of 2*fp.Size bytes.
TEXT ·doubleBmi2Adx(SB),NOSPLIT,$192
    MOVQ work+0(FP), regWork
    addSub(x1,z1)
    integerSqrAdx(b0,x1)
    integerSqrAdx(b1,z1)
