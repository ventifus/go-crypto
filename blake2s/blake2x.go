...
func (x *xof) Reset() {
	...
	binary.LittleEndian.PutUint16(x.cfg[12:], x.length)    // XOF lengthh
...
func (x *xof) Read(p []byte) (n int, err error) {
	...
	if n > int(x.remaining) {
		n = int(x.remaining)
		p = p[:n]
		err = errors.New("blake2s: hash limit exceeded")
	}
	...
	if x.offset > 0 {
		...
		if n < dif {
			...
			x.remaining -= uint16(n)
			return
		}
		copy(p, x.block[x.offset:])
		p = p[dif:]
		x.offset = 0
		x.remaining -= uint16(dif)
	}
	...
	for len(p) >= Size {
		...
		copy(p, x.block[:])
		x.remaining -= uint16(Size)
		p = p[Size:]
	}

	if nn := len(p); nn > 0 {
		if x.remaining < uint16(Size) {
