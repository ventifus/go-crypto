...
func TestCipherEncryptFlexible(t *testing.T) {
	for i, tt := range CryptTests_16 {
		...
		out := make([]byte, len(tt.plainText))
		c.EncryptFlexible(out, tt.plainText, 16)
	...
	for i, tt := range CryptTests { // Test normal test vectors too
		c, err := NewCipher(tt.key)
		if err != nil {
			t.Errorf("NewCipher(%d bytes), vector %d = %s", len(tt.key), i, err)
			continue
		...
		out := make([]byte, len(tt.plainText))
		c.EncryptFlexible(out, tt.plainText, 64)
...
func TestCipherDecryptFlexible(t *testing.T) {
	for i, tt := range CryptTests_16 {
		c, err := NewCipher(tt.key)
		if err != nil {
			t.Errorf("NewCipher(%d bytes), vector %d = %s", len(tt.key), i, err)
		...
		out := make([]byte, len(tt.cipherText))
		c.DecryptFlexible(out, tt.cipherText, 16)
	...
	for i, tt := range CryptTests {
		...
		out := make([]byte, len(tt.cipherText))
		c.DecryptFlexible(out, tt.cipherText, 64)
