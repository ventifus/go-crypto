...
func addTestKeyConstrained(t *testing.T, a Agent, keyName string, ttl uint32, confirm bool) {
	err := a.Add(AddedKey{
		PrivateKey:       testPrivateKeys[keyName],
		Comment:          keyName,
		LifetimeSecs:     ttl,
...
func TestExpire(t *testing.T) {
	...
	addTestKeyConstrained(t, k, "rsa", 1, false)
...
func TestConfirm(t *testing.T) {
	...
	addTestKeyConstrained(t, k, "rsa", 0, true)

	pub := testPublicKeys["rsa"]
	if _, err := k.Sign(pub, []byte("hello")); err == nil {
		t.Fatalf("agent signed key without confirmation")
	}
}
