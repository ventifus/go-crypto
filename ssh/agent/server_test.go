...
func TestParseConstraints(t *testing.T) {
	...
	if lifetimeSecs != msg.LifetimeSecs {
		t.Errorf("expect lifetime %v, found %v", msg.LifetimeSecs, lifetimeSecs)
	}
	...
	var data []byte
	for i := 0; i < 10; i++ {
		var ext = constrainExtensionAgentMsg{
	...
	for i := 0; i < 10; i++ {
		...
		if !reflect.DeepEqual(ext, extensions[i]) {
			t.Errorf("expect extension %v, found %v", ext, extensions[i])
		}
	}
