...
type Agent interface {
	...
	// Insert adds a private key to the agent with the given
...
type rsaKeyConstrainedMsg struct {
	Type        string `sshtype:"25"`
	N           *big.Int
	E           *big.Int
	D           *big.Int
	Iqmp        *big.Int // IQMP = Inverse Q Mod P
...
type dsaKeyConstrainedMsg struct {
	Type        string `sshtype:"25"`
	P           *big.Int
	Q           *big.Int
	G           *big.Int
...
type ecdsaKeyConstrainedMsg struct {
	Type        string `sshtype:"25"`
	Curve       string
	KeyBytes    []byte
	D           *big.Int
...
func (c *client) insertKey(s interface{}, comment string, constraints []byte) error {
	...
	case *rsa.PrivateKey:
		...
		if len(constraints) == 0 {
			...
			req = ssh.Marshal(rsaKeyConstrainedMsg{
				Type:        ssh.KeyAlgoRSA,
				N:           k.N,
				E:           big.NewInt(int64(k.E)),
				D:           k.D,
	...
	case *dsa.PrivateKey:
		if len(constraints) == 0 {
			...
			req = ssh.Marshal(dsaKeyConstrainedMsg{
				Type:        ssh.KeyAlgoDSA,
				P:           k.P,
				Q:           k.Q,
				G:           k.G,
	...
	case *ecdsa.PrivateKey:
		...
		if len(constraints) == 0 {
			...
			req = ssh.Marshal(ecdsaKeyConstrainedMsg{
				Type:        "ecdsa-sha2-" + nistID,
				Curve:       nistID,
				KeyBytes:    elliptic.Marshal(k.Curve, k.X, k.Y),
				D:           k.D,
...
type rsaCertConstrainedMsg struct {
	Type        string `sshtype:"25"`
	CertBytes   []byte
	D           *big.Int
	Iqmp        *big.Int // IQMP = Inverse Q Mod P
...
func (c *client) insertCert(s interface{}, cert *ssh.Certificate, comment string, constraints []byte) error {
	...
	case *rsa.PrivateKey:
		...
		if len(constraints) == 0 {
			...
			req = ssh.Marshal(rsaCertConstrainedMsg{
				Type:        cert.Type(),
				CertBytes:   cert.Marshal(),
				D:           k.D,
				Iqmp:        k.Precomputed.Qinv,
