...
func TestAuthMethodGSSAPIWithMICInitSecContextFail(t *testing.T) {
	config := &ClientConfig{
		User: "testuser	",
		Auth: []AuthMethod{
			GSSAPIWithMICAuthMethod(
	...
	expectedErr := errors.New("ssh: handshake failed: InitSecContext err")
...
func TestAuthMethodGSSAPIWithMICGetMICFail(t *testing.T) {
	config := &ClientConfig{
		User: "testuser",
		Auth: []AuthMethod{
			GSSAPIWithMICAuthMethod(
	...
	expectedErr := errors.New("ssh: handshake failed: GetMIC err")
	if err := tryAuthWithGSSAPIWithMICConfig(t, config, gssAPIWithMICConfig); err != nil && err.Error() != expectedErr.Error() {
		t.Fatalf("client: Got %v, want %v", err, expectedErr)
	}
}
...
func TestAuthMethodGSSAPIWithMICAcceptSecContextFail(t *testing.T) {
	config := &ClientConfig{
		User: "testuser",
		Auth: []AuthMethod{
			GSSAPIWithMICAuthMethod(
	...
	_, serverErrs := tryAuthBothSides(t, config, gssAPIWithMICConfig)
	if serverErrs == nil {
	...
	expectedErr := errors.New("AcceptSecContext err")
...
func TestAuthMethodGSSAPIWithMICVerifyMICFail(t *testing.T) {
	config := &ClientConfig{
		User: "testuser",
		Auth: []AuthMethod{
			GSSAPIWithMICAuthMethod(
	...
	expectedErr := errors.New("VerifyMIC err")
	if serverErrs[1].Error() != expectedErr.Error() {
		t.Fatalf("server: Got %v, want %v", serverErrs[1], expectedErr)
	}
}
...
func TestAuthMethodGSSAPIWithMICGetSrcNameFail(t *testing.T) {
	config := &ClientConfig{
		User: "testuser",
		Auth: []AuthMethod{
			GSSAPIWithMICAuthMethod(
	...
	if len(serverErrs) != 2 {
		t.Fatalf("server: got length %d, want 2, errs is %v", len(serverErrs), serverErrs)
	}
	expectedErr := errors.New("GetSrcName err")
	if serverErrs[1].Error() != expectedErr.Error() {
		t.Fatalf("server: Got %v, want %v", serverErrs[1], expectedErr)
	}
}
