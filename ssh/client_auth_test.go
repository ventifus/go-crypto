...
func TestAuthMethodGSSAPIWithMIC(t *testing.T) {
	...
	testcases := []*testcase{
		...
		},
		{
			...
			clientWantErr: "ssh: handshake failed: ssh: unable to authenticate, attempted methods [none gssapi-with-mic], no supported methods remain",
		},
		{
			config: &ClientConfig{
				User: "testuser",
	...
	for i, c := range testcases {
		...
		if (c.serverWantErr != "") != (len(serverErrs) == 2 && serverErrs[1] != nil) {
