	...
	testcases := []*testcase{
		{
				...
				Auth: []AuthMethod{
					GSSAPIWithMICAuthMethod(
						&FakeClient{
							exchanges: []*exchange{
								{
									outToken:      []byte("client-valid-token-1"),
									expectedToken: nil,
								},
								{
									outToken:      []byte{},
									expectedToken: []byte("server-valid-token-1"),
								},
							},
							mic:   []byte("valid-mic"),
							round: 2,
			...
			gssConfig: &GSSAPIWithMICConfig{
				...
				Server: &FakeServer{
					exchanges: []*exchange{
						{
							outToken:      []byte("server-valid-token-1"),
							expectedToken: []byte("client-valid-token-1"),
						},
					},
					round:       1,
		...
		},
		{
			...
			gssConfig: &GSSAPIWithMICConfig{
				AllowLogin: func(conn ConnMetadata, srcName string) (*Permissions, error) {
					return nil, fmt.Errorf("use is not allowed to login")
				},
				Server: &FakeServer{
					exchanges: []*exchange{
						{
			...
			serverWantErr: "use is not allowed to login",
		},
		{
			config: &ClientConfig{
				User: "testuser",
			...
			gssConfig: &GSSAPIWithMICConfig{
				...
				Server: &FakeServer{
					exchanges: []*exchange{
						{
							outToken:      []byte("server-invalid-token-1"),
							expectedToken: []byte("client-valid-token-1"),
						},
					},
					round:       1,
			...
			clientWantErr: "ssh: handshake failed: token is invalid",
		},
		{
			config: &ClientConfig{
				User: "testuser",
			...
			serverWantErr: "VerifyMIC fail",
		},
	}

	for _, c := range testcases {
		clientErr, serverErrs := tryAuthBothSides(t, c.config, c.gssConfig)
		if c.clientWantErr == "" && c.serverWantErr == "" {
			if clientErr != nil {
				t.Fatalf("client got %v, want nil", clientErr)
			}
		}
		if c.clientWantErr != "" {
			if clientErr != nil && clientErr.Error() != c.clientWantErr {
				t.Fatalf("client  got %v, want %s", clientErr, c.clientWantErr)
			}
		}
		found := false
		var errStrings []string
		if c.serverWantErr != "" {
			...
			if !found {
				t.Errorf("server got error %q, want substring %q", errStrings, c.serverWantErr)
			}
		}
	}
}
