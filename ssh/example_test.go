...
func ExampleNewServerConn_PublicKeyAuth() {
	// Our goal is to parse in all authorized keys and then compare
	// the public keys of connections that the server receives.
	authorizedKeysBytes, err := ioutil.ReadFile("authorized_keys")
	if err != nil {
		log.Fatalf("Failed to authorized_keys: %v", err)
	}

	authorizedKeys := map[ssh.PublicKey]struct{}{}
	...
	config := &ssh.ServerConfig{
		PublicKeyCallback: func(c ssh.ConnMetadata, pubKey ssh.PublicKey) (*ssh.Permissions, error) {
			_, authorized := authorizedKeys[pubKey]
			if authorized {
				return nil, nil
			}
