...
func (kex *mlkem768WithCurve25519sha256) Client(c packetConn, rand io.Reader, magics *handshakeMagics) (*kexResult, error) {
	...
	var mlkemDk *mlkem.DecapsulationKey768
	var err error

	if err = c25519kp.generate(rand); err != nil {
		return nil, err
	}
	if mlkemDk, err = mlkem.GenerateKey768(); err != nil {
		return nil, err
	}
	...
	if len(reply.EphemeralPubKey) != mlkem.CiphertextSize768+32 {
		return nil, fmt.Errorf("ssh: peer's ML-KEM768/curve25519 public value has wrong length: %d", len(reply.EphemeralPubKey))
	}

	// Perform KEM decapsulate operation to obtain shared key from ML-KEM.
	var mlkem768Secret []byte
	if mlkem768Secret, err = mlkemDk.Decapsulate(reply.EphemeralPubKey[:mlkem.CiphertextSize768]); err != nil {
		return nil, err
	}
...
func (kex *mlkem768WithCurve25519sha256) Server(c packetConn, rand io.Reader, magics *handshakeMagics, priv AlgorithmSigner, algo string) (result *kexResult, err error) {
	...
	if len(kexInit.ClientPubKey) != mlkem.EncapsulationKeySize768+32 {
		return nil, fmt.Errorf("ssh: peer's ML-KEM768/curve25519 public value has wrong length: %d", len(kexInit.ClientPubKey))
	}

	encapsulationKey, err := mlkem.NewEncapsulationKey768(kexInit.ClientPubKey[:mlkem.EncapsulationKeySize768])
	if err != nil {
		return nil, fmt.Errorf("ssh: peer's ML-KEM768/curve25519 public is not valid: %w", err)
	}
