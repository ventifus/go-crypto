...
func init() {
	...
	for t, k := range testdata.PEMBytes {
		...
		if v, ok := testSigners[t].(*defaultAlgorithmSigner); ok {
			switch t {
			case "rsa-sha2-256":
				testSigners[t] = &defaultAlgorithmSigner{v, SigAlgoRSASHA2256}
			case "rsa-sha2-512":
				testSigners[t] = &defaultAlgorithmSigner{v, SigAlgoRSASHA2512}
			}
		}
		if err != nil {
