...
func (f *FakeClient) InitSecContext(target string, token []byte, isGSSDelegCreds bool) (outputToken []byte, needContinue bool, err error) {
	if token == nil {
		if f.exchanges[f.i].expectedToken != nil {
			err = errors.New("token is invalid")
		} else {
			outputToken = f.exchanges[f.i].outToken
		}
	} else {
...
//	t                    *testing.T
//	fakeAcceptSecContext func(token []byte) (outputToken []byte, srcName string, needContinue bool, err error)
//	fakeVerifyMIC        func(micField []byte, micToken []byte) error
//}
//
//func (f *FakeGSSAPIServer) AcceptSecContext(token []byte) (outputToken []byte, srcName string, needContinue bool, err error) {
...
//	}
//	return []byte{2}, "testuser@DOMAIN", false, nil
//}
//
