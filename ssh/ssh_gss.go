...
type GSSAPIClient interface {
	...
	// See RFC 2743 section 2.3.1.
	GetMIC(micFiled []byte) ([]byte, error)
	// DeleteSecContext is called to ensure
	// that the sec context created by InitSecContext has been removed, if
...
type GSSAPIServer interface {
	...
	// DeleteSecContext is called to ensure
	// that the sec context created by AcceptSecContext has been removed, if
	// errors occur in later phases of context establishment, or when an
	// established context is no longer required.
	// See RFC 2743 section 2.2.3.
	DeleteSecContext() error
...
func buildMIC(sectionID string, username string, service string, authMethod string) []byte {
	out := make([]byte, 0, 0)
	out = appendString(out, sectionID)
