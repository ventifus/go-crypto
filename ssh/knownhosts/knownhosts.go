...
// Package knownhosts implements a parser for the OpenSSH's
...
func (l *hostPattern) match(a addr) bool {
	if strings.Contains(l.addr.host, "?") || strings.Contains(l.addr.host, "*") {
		// These are rejected on parse, but we're paranoid.
...
func (l *keyDBLine) String() string {
	...
	return c + strings.Join(ss, ",") + " " + serialize(l.knownKey)
}
...
func (db *hostKeyDB) IsAuthority(remote ssh.PublicKey) bool {
	for _, l := range db.lines {
		...
		if l.cert && keyEq(l.knownKey, remote) {
			return true
		}
	}
	return false
...
func (db *hostKeyDB) parseLine(line []byte) error {
	...
	for _, p := range patterns {
		...
		if strings.Contains(a.host, "?") || strings.Contains(a.host, "*") {
			...
			return errors.New("knownhosts: wildcards not implemented.")
		}
...
func (db *hostKeyDB) check(address string, remote net.Addr, remoteKey ssh.PublicKey) error {
	if _, ok := db.revoked[string(remoteKey.Marshal())]; ok {
		return &RevokedError{}
	}
...
func (db *hostKeyDB) checkAddrs(addrs []addr, remoteKey ssh.PublicKey) error {
	...
	for _, l := range db.lines {
		if l.match(addrs) {
			if _, ok := knownKeys[l.knownKey.Type()]; !ok {
				knownKeys[l.knownKey.Type()] = l.knownKey
			}
		}
	}
	...
	if known := knownKeys[remoteKey.Type()]; known == nil || !keyEq(known, remoteKey) {
		return keyErr
	}
