// Package knownhosts implements a full-fledged parser for the OpenSSH's knownhosts host key database.
package knownhosts
...
func (db *hostKeyDB) check(address string, remote net.Addr, key ssh.PublicKey) error {
	...
	// TODO(hanwen): should we do something intelligent if there is no IP address?
...
func (db *hostKeyDB) Read(r io.Reader) error {
	...
	for scanner.Scan() {
		...
		if err := db.parseLine(txt); err != nil {
			// TODO(hanwen): should we try to read the key db partially?
			return err
		}
	}
	return scanner.Err()
...
func New(files ...string) (ssh.HostKeyCallback, error) {
	// TODO(hanwen): should we provide a mutable interface to hostKeyDB?
	db := newHostKeyDB()
	for _, f := range files {
		f, err := os.Open(f)
		if err != nil {
