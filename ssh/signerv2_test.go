...
func TestNewSignerV2(t *testing.T) {
	for name, key := range testdata.PEMBytes {
		...
		k, err := ParsePrivateKeyV2(key, nil)
		if err != nil {
			t.Fatalf("unable to parse private key %q: %v", name, err)
		}
		signer, err := NewSignerV2(k)
		if err != nil {
			t.Fatalf("NewSignerV2(%#v): %v", k, err)
		}
...
func TestCertSignerV2(t *testing.T) {
	key, err := ParsePrivateKeyV2(testdata.PEMBytes["ed25519"], nil)
	if err != nil {
		t.Fatalf("unable to parse certificate key: %v", err)
	}
	certKey, err := NewSignerV2(key)
	if err != nil {
		t.Fatalf("unable to create certificate signer: %v", err)
	}
	authorityKey, err := ParsePrivateKeyV2(testdata.PEMBytes["ecdsa"], nil)
	if err != nil {
		t.Fatalf("unable to parse authority key: %v", err)
	}
	authority, err := NewSignerV2(authorityKey)
	if err != nil {
		t.Fatalf("unable to create authority: %v", err)
	}
	...
	if err := cert.SignCertV2(rand.Reader, authority); err != nil {
		t.Fatalf("SignCertV2: %v", err)
	}

	_, err = NewCertificateSignerV2(cert, certKey)
	if err != nil {
