...
type muxCallbacks struct {
	...
	hostKeysUpdate HostKeysUpdateCallback
}
...
func handleHostKeys00Request(req *Request, callback HostKeysUpdateCallback) {
	var publicKeys []HostKeyUpdate
	...
	for {
		...
		publicKeys = append(publicKeys, HostKeyUpdate{pubKey})
		payload = rest
	}
	req.Reply(false, nil)
	callback(publicKeys)
...
func (m *mux) handleGlobalPacket(packet []byte) error {
	...
	case *globalRequestMsg:
		...
		case "hostkeys-00@openssh.com":
			if m.callbacks != nil && m.callbacks.hostKeysUpdate != nil {
				go handleHostKeys00Request(req, m.callbacks.hostKeysUpdate)
			} else {
				m.incomingRequests <- req
			}
		default:
