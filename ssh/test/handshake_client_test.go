...
func (test *clientTest) run(t *testing.T, write bool) {
	...
	if write {
		...
		clientConn = newReplayingConn(t, flows, false)
	}
...
func recordingsClientConfig() (*ssh.ClientConfig, error) {
	config := clientConfig()
	config.Rand = sha3.NewShake128()
	signer, err := ssh.NewSignerWithAlgorithms(testSigners["rsa"].(ssh.AlgorithmSigner), []string{ssh.KeyAlgoRSASHA256})
	if err != nil {
		return nil, err
	}
