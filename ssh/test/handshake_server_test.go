...
func (test *serverTest) connFromCommand(t *testing.T) (conn *recordingConn, err error) {
	...
	filename := "id_rsa"
	writeFile(filepath.Join(dir, filename), testdata.PEMBytes["rsa-openssh-format"])
	writeFile(filepath.Join(dir, filename+".pub"), ssh.MarshalAuthorizedKey(testPublicKeys["rsa-openssh-format"]))
	var args []string
	...
	cmd := testenv.Command(t, "ssh", args...)
	cmd.Stdin = nil
...
func (test *serverTest) run(t *testing.T, write bool) {
	...
	if write {
		...
		serverConn = newReplayingConn(t, flows, false)
	}
