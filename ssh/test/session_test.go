...
func testOneCipher(t *testing.T) {
	server := newServer(t)
	defer server.Shutdown()
	conf := clientConfig()
	...
	if _, _, err := conn.Conn.SendRequest("drop-me", false, make([]byte, want)); err != nil {
	...
	out, err := session.Output(fmt.Sprintf("dd if=/dev/zero of=/dev/stdout bs=%d count=1", want))
	if err != nil {
		t.Fatalf("Output: %v", err)
	}

	if len(out) != want {
		t.Fatalf("got %d bytes, want %d bytes", len(out), want)
	}
}
