...
func TestParseEncryptedPrivateKeysFails(t *testing.T) {
	for i, tt := range testdata.PEMEncryptedKeys {
		...
		if _, ok := err.(*PassphraseMissingError); !ok {
			t.Errorf("#%d key %s: got error %q, want PassphraseMissingError", i, tt.Name, err)
		}
	}
}
...
func TestParseEncryptedPrivateKeysWithPassphrase(t *testing.T) {
	...
	for _, tt := range testdata.PEMEncryptedKeys {
		...
		if err, ok := err.(*PassphraseMissingError); !ok {
			t.Errorf("%s: got error %q, want PassphraseMissingError", tt.Name, err)
		} else if tt.IncludesPublicKey {
			if err.PublicKey == nil {
				t.Errorf("%s: expected PassphraseMissingError.PublicKey not to be nil", tt.Name)
				continue
			}
