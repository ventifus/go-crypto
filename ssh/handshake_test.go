...
func testHandshakeErrorHandlingN(t *testing.T, readLimit, writeLimit int) {
	...
	serverConf.SetDefaults()
	serverConn := newHandshakeTransport(&errorKeyingTransport{a, readLimit, writeLimit}, &serverConf,
		[]byte{'a'}, []byte{'b'})
	...
	clientConf.SetDefaults()
	clientConn := newHandshakeTransport(&errorKeyingTransport{b, -1, -1}, &clientConf,
		[]byte{'a'}, []byte{'b'})
