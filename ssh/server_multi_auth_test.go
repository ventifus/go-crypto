...
func TestAuthCallbackReturningPartialSuccessAndChangeAuthMethods(t *testing.T) {
	serverConfig := &ServerConfig{
		PasswordCallback: func(conn ConnMetadata, password []byte) (*Permissions, error) {
			errPartial := &PartialSuccessError{
				Next: ServerAuthCallbacks{
					PasswordCallback: func(conn ConnMetadata, password []byte) (*Permissions, error) {
						return nil, nil
					},
			...
			errChange := &ChangeAuthMethodsError{
				Next: ServerAuthCallbacks{
					PublicKeyCallback: func(conn ConnMetadata, key PublicKey) (*Permissions, error) {
						return nil, nil
					},
			...
			return nil, errors.Join(errPartial, errChange)
		},
	}
	...
	_, err := doClientServerAuth(t, serverConfig, clientConfig)
	if err == nil {
		t.Fatalf("authentication that returned an invalid callback was successful!")
	}
}
