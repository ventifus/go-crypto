...
// Defines "supported" and "preferred" algorithms. The supported algorithms are
// those implemented by this package excluding those with security issues. The
// preferred algorithms are those used by default, they may differ from the
// supported ones for backward compatibility reasons.
var (
...
func (c *Config) SetDefaults() {
	...
	for _, k := range c.KeyExchanges {
		...
		if kexAlgoMap[k] != nil {
			kexs = append(kexs, k)
			if k == KexAlgoCurve25519SHA256 && !contains(c.KeyExchanges, kexAlgoCurve25519SHA256LibSSH) {
				kexs = append(kexs, kexAlgoCurve25519SHA256LibSSH)
			}
		}
	}
	c.KeyExchanges = kexs
