...
func TestVerifyHostKeySignature(t *testing.T) {
	for _, tt := range []struct {
		...
		{"rsa", KeyAlgoRSASHA2256, KeyAlgoRSASHA2256, ""},
		{"rsa", KeyAlgoRSASHA2512, KeyAlgoRSA, `ssh: invalid signature algorithm "ssh-rsa", expected "rsa-sha2-512"`},
		...
		err = verifyHostKeySignature(key, tt.algo, &result)
		if err != nil {
			if tt.wantError == "" || !strings.Contains(err.Error(), tt.wantError) {
				t.Errorf("got error %q, missing %q", err.Error(), tt.wantError)
			}
		} else if tt.wantError != "" {
