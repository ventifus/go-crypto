...
func pickSignatureAlgorithm(signer Signer, extensions map[string][]byte) (MultiAlgorithmSigner, string, error) {
	...
	getFallbackAlgo := func() (string, error) {
		...
		if !contains(as.Algorithms(), underlyingAlgo(keyFormat)) {
			return "", fmt.Errorf("ssh: no common public key signature algorithm, server algorithm %q, signer algorithms: %v",
				underlyingAlgo(keyFormat), as.Algorithms())
		}
...
func (cb publicKeyCallback) auth(session []byte, user string, c packetConn, rand io.Reader, extensions map[string][]byte) (authResult, []string, error) {
	...
	for _, signer := range signers {
		...
		if err != nil {
			// If we cannot negotiate a signature algorithm store the error so
			// we can return it to provide a more meaningful message if no other
			// signers work.
			errSigAlgo = err
			continue
		}
