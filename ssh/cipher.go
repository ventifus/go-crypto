...
var cipherModes = map[string]*streamCipherMode{
	...
	// AEAD cipher are special-case. If we add any more non-stream
...
// chacha20-poly1305 is described here:
//
//   https://tools.ietf.org/html/draft-josefsson-ssh-chacha20-poly1305-openssh-00
//
// in addition, even for stream ciphers, packets are padded as
// described in https://tools.ietf.org/html/rfc4253 section 6.
...
func (c *chacha20Poly1305Cipher) readPacket(seqNum uint32, r io.Reader) ([]byte, error) {
	...
	if _, err := r.Read(c.buf[:4]); err != nil {
		return nil, err
	}

	var lenBytes [4]byte
	chacha20.XORKeyStream(lenBytes[:], c.buf[:4], &counter, &c.lengthKey)
	...
	packetEnd := length + poly1305Len + 4
	contentEnd := 4 + length
...
func (c *chacha20Poly1305Cipher) writePacket(seqNum uint32, w io.Writer, rand io.Reader, payload []byte) error {
	...
	totalLength := 4 + 1 + len(payload) + padding + poly1305Len
