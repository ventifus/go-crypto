...
func (e *ChangeAuthMethodsError) Error() string {
	// This error string is not returned to clients. We return
	// SSH_MSG_USERAUTH_FAILURE which contains the authentication methods that
	// can continue.
	return "ssh: authentication can continue"
}
	...
	for {
		...
		if errors.As(authErr, &changeAuthMethods) {
			if newAuthCallbacks != nil {
				return nil, errors.New("ssh: internal error, the auth callback returned an error implementing both PartialSuccessError and ChangeAuthMethodsError")
			}
