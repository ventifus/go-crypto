...
func (s *connection) serverAuthenticate(config *ServerConfig) (*Permissions, error) {
	...
	// changed if a PartialSuccess is returned.
...
userAuthLoop:
	for {
		...
		if s.user != "" && s.user != userAuthReq.User && partialSuccessReturned {
		...
		case "none":
			...
			if authFailures == 0 && authErr != nil {
				// PartialSuccessError will not increase authFailures.
				if _, ok := authErr.(*PartialSuccessError); !ok {
					authFailures--
				}
			}
		case "password":
		...
		if partialSuccess, ok := authErr.(*PartialSuccessError); ok {
			...
			// Send back a partial success message to the user if the
			// authentication method is different from "none".
