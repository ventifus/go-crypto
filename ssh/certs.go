...
func parseTuples(in []byte) (map[string]string, error) {
	...
	for len(in) > 0 {
		...
		// 8 extra bytes to account for the key and value length fields
		if len(in) < int(keylen+8) {
			return nil, errShortRead
		}
		vallen := binary.BigEndian.Uint32(in[keylen+4:])
		end := int(keylen + vallen + 8)
		if len(in) < end {
			return nil, errShortRead
		}
		if err := Unmarshal(in[:end], &tup); err != nil {
			return nil, errShortRead
		}
		name = tup.Key
		if len(tup.Value) > 0 {
			val := optionsTupleValue{}
			if err := Unmarshal(tup.Value, &val); err != nil {
				return nil, errShortRead
			}
			value = val.Value
		} else {
