...
func parseTuples(in []byte) (map[string]string, error) {
	...
	for len(in) > 0 {
		...
		var val []byte
		// the next element is either an non-empty string with two length fields
		// or four bytes of zeros for an empty value
		val, _, ok = parseString(in)
		if !ok {
			return nil, errShortRead
		}
		// move forward the "in" tape marker four bytes to account
		// for the read length index we just read
		in = in[4:]
		if len(val) > 0 {
