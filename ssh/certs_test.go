...
func generateCert(pub crypto.PublicKey, signer Signer) *Certificate {
	pubKey, _ := NewPublicKey(pub)
	cert := &Certificate{
		CertType:    UserCert,
...
func TestCertTypes(t *testing.T) {
	var testVars = []struct {
		name string
		keys func() (crypto.PrivateKey, crypto.PublicKey)
	}{
		{
	...
	for _, m := range testVars {
		t.Run(m.name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("error generating signer from private key: %v", err)
			}
			cert := generateCert(pub, signer)
