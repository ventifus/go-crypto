...
type channel struct {
	...
	readyToWrite     bool
	writeBufferReady chan *writeRequest
	writeBuffer      []byte
}
...
func (ch *channel) writeLoop() {
	for writeReq := range ch.writeBufferReady {
		// It is safe to use ch.writeBuffer because, even if we return from
		// writePacket due to a deadline, ch.writeBuffer cannot be modified
		// until the current write finishes and ch.readyToWrite is set.
		err := ch.mux.conn.writePacket(ch.writeBuffer)
		ch.writeMu.Lock()
...
func (ch *channel) writePacket(header, data []byte) error {
	...
	for {
		...
		if ch.readyToWrite {
			...
			isClose := ch.writeBuffer[0] == msgChannelClose
			ch.sentClose = isClose
			writeReq := &writeRequest{}
			ch.writeBufferReady <- writeReq

			ch.writeCond.Wait()
			// Wait can end because the write completes, a deadline expires,
			// sentClose is set in ch.close(). We don't use ch.deadlineReached
			// becuase it can be set to false by a different goroutine that
			// acquires writeMu before this goroutine.
			if !writeReq.done {
				if ch.sentClose && !isClose {
					return io.EOF
				}
				return os.ErrDeadlineExceeded
			}
			// The write has been done, so we return the write error, if any.
			return writeReq.err
		}
...
func (m *mux) newChannel(chanType string, direction channelDirection, extraData []byte) *channel {
	ch := &channel{
		...
		extraData:        extraData,
		mux:              m,
		readyToWrite:     true, // initially the channel is ready to write
		writeBufferReady: make(chan *writeRequest),
	}
