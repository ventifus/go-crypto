...
func (ch *channel) writeLoop() {
	...
	for range ch.writeBufferReady {
		...
		if ch.writeBuffer[0] == msgChannelClose {
			ch.setClose()
			return
		}
		ch.setReadyToWrite()
...
func (m *mux) newChannel(chanType string, direction channelDirection, extraData []byte) *channel {
	ch := &channel{
		...
		extraData:        extraData,
		mux:              m,
		writeBufferReady: make(chan struct{}),
	}
