...
func (ch *channel) WriteExtended(data []byte, extendedCode uint32) (n int, err error) {
	deadline := ch.writeDeadline.Load()
	...
	for len(data) > 0 {
		...
		if deadline > 0 {
			...
			reserveCh := make(chan winReserveResult, 1)

			go func() {
				space, err := ch.remoteWin.reserve(space)
				reserveCh <- winReserveResult{space, err}
			}()
			...
			case res := <-reserveCh:
				...
				err = res.err
			case <-ctx.Done():
				// Calling deadline() will unblock the above goroutine.
				ch.remoteWin.deadline()
				res := <-reserveCh
				space = res.space
				err = res.err
			}
