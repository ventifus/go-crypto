...
func TestBlockMode(t *testing.T, encrypter, decrypter MakeBlockMode) {
	for _, keylen := range []int{128, 192, 256} {
		t.Run(fmt.Sprintf("AES-%d", keylen), func(t *testing.T) {
			...
			testBlockModePair(t, encrypter, decrypter, c, iv)
		})
	}
...
func testBlockModePair(t *testing.T, e, d MakeBlockMode, b cipher.Block, iv []byte) {
	t.Run("Encryption", func(t *testing.T) {
		testBlockMode(t, e, b, iv)
	})
	t.Run("Decryption", func(t *testing.T) {
		testBlockMode(t, d, b, iv)
	})
