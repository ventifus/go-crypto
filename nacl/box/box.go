...
Functionality aside form anonymous sealing/opening is interoperable with NaCl:
https://nacl.cr.yp.to/box.html. Anonymous sealing/opening is an extension
of NaCl defined by and interoperable with libsodium:
...
const (
	...
	// AnonymousOverhead is the number of bytes of overhead when using
	// SealAnonymous.
...
// overlap it. This differs from Seal() in that that sender is not required to
...
func SealAnonymous(out, message []byte, recipient *[32]byte, rand io.Reader) ([]byte, error) {
	...
	nonce, err := sealNonce(ephemeralPub, recipient)
	if err != nil {
		return nil, err
	}
...
func OpenAnonymous(out, box []byte, key *[32]byte) (message []byte, ok bool) {
	...
	nonce, err := sealNonce(ephemeralPub, pub)
	if err != nil {
		return nil, false
	}

	return Open(out, box[32:], nonce, ephemeralPub, key)
}

// sealNonce generates a 24 byte nonce that is a blake2b digest of the
// ephemeral public key and the receiver's public key.
func sealNonce(ephemeralPub, peersPublicKey *[32]byte) (*[24]byte, error) {
