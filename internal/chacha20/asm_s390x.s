...
// func xorKeyStreamVX(dst, src []byte, counter *[4]uint32, key *[8]uint32, buf *[256]byte)
TEXT ·xorKeyStreamVX(SB), NOSPLIT, $256-64
	LMG    dst+0(FP), R5, R6  // R5=&dst[0] R6=len(dst)
	LMG    src+24(FP), R7, R8 // R7=&src[0] R8=len(src)
	CMPBLT R6, R8, fault      // assert len(dst) >= len(src)
	CMPBEQ R8, $0, fault      // assert len(src) > 0
...
loop:
	...
	ROTATE(1*4, X1, X5,  X9, X13)
	ROTATE(3*4, X3, X7, X11, X15)
	ROTATE(2*4, X2, X6, X10, X14)

	ROUND(X0, X1, X2, X3, X4, X5, X6, X7, X8, X9, X10, X11, X12, X13, X14, X15)
	ROTATE(3*4, X1, X5,  X9, X13)
	ROTATE(1*4, X3, X7, X11, X15)
	ROTATE(2*4, X2, X6, X10, X14)
