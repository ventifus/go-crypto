...
// Cipher is a stateful instance of ChaCha20 using a particular
// key and nonce.
type Cipher struct {
	key     [8]uint32
	nonce   [3]uint32
...
// The initial counter value is set to 0. *Cipher implements the
...
func (s *Cipher) XORKeyStream(dst, src []byte) {
	...
	s1, s5, s13, s9 := qr(j1, s.key[1], s.nonce[0], s.key[5])
	s2, s6, s14, s10 := qr(j2, s.key[2], s.nonce[1], s.key[6])
	s3, s7, s15, s11 := qr(j3, s.key[3], s.nonce[2], s.key[7])

	n := len(src)
	src, dst = src[:n:n], dst[:n:n] // BCE hint
	for i := 0; i < n; i += 64 {
		// calculate the remainder of the first round
		s0, s4, s12, s8 := qr(j0, s.key[0], s.counter, s.key[4])
