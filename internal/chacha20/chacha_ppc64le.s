...
loop_vmx:
	...
	SUB X1, X2, OUT // in misaligned case edges
...
loop:
	...
	// Add the original state over the end state
	ADD $0x61707865, CON0, X0
	ADD $0x3320646e, CON1, X1
	ADD $0x79622d32, CON2, X2
	ADD $0x6b206574, CON3, X3

	SUBECC TEMP, TEMP, TEMP
	ADD KEY0, TMP0, X4
	MOVWZ 16(KEY), TMP0
	ADD KEY1, TMP1, X5
	MOVWZ 20(KEY), TMP1
...
tail:
	...
	// Prepare the address for ptr++ inside the loop bellow
	SUB $1, INP, INP
	SUB $1, OUT, OUT
	ADD $OFFSET-1, R1, TMP0
	MOVD LEN, CTR

	MOVW X0, x0+(0+OFFSET)(SP)
	MOVW X1, x1+(4+OFFSET)(SP)
	MOVW X2, x2+(8+OFFSET)(SP)
	MOVW X3, x3+(12+OFFSET)(SP)
	MOVW X4, x4+(16+OFFSET)(SP)
	MOVW X5, x5+(20+OFFSET)(SP)
	MOVW X6, x6+(24+OFFSET)(SP)
	MOVW X7, x7+(28+OFFSET)(SP)
	MOVW X8, x8+(32+OFFSET)(SP)
	MOVW X9, x9+(36+OFFSET)(SP)
	MOVW X10, x10+(40+OFFSET)(SP)
	MOVW X11, x11+(44+OFFSET)(SP)
	MOVW X12, x12+(48+OFFSET)(SP)
	MOVW X13, x13+(52+OFFSET)(SP)
	MOVW X14, x14+(56+OFFSET)(SP)
	MOVW X15, x15+(60+OFFSET)(SP)

loop_tail:
	MOVBZU 1(INP), CNT0 // Copy byte from input and increment its pointer
	MOVBZU 1(TMP0), X0  // Copy byte from temp and increment its pointer
	XOR X0, CNT0, CNT1  // XOR the two values above
	MOVBU CNT1, 1(OUT)  // Save to output the byte stored in CNT1
	BC 0x10, 0, loop_tail

	// Wipe stack pointer
	MOVW R1, x0+(0+OFFSET)(SP)
	MOVW R1, x1+(4+OFFSET)(SP)
	MOVW R1, x2+(8+OFFSET)(SP)
	MOVW R1, x3+(12+OFFSET)(SP)
	MOVW R1, x4+(16+OFFSET)(SP)
	MOVW R1, x5+(20+OFFSET)(SP)
	MOVW R1, x6+(24+OFFSET)(SP)
	MOVW R1, x7+(28+OFFSET)(SP)
	MOVW R1, x8+(32+OFFSET)(SP)
	MOVW R1, x9+(36+OFFSET)(SP)
	MOVW R1, x10+(40+OFFSET)(SP)
	MOVW R1, x11+(44+OFFSET)(SP)
	MOVW R1, x12+(48+OFFSET)(SP)
	MOVW R1, x13+(52+OFFSET)(SP)
	MOVW R1, x14+(56+OFFSET)(SP)
	MOVW R1, x15+(60+OFFSET)(SP)
