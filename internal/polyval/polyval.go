...
//go:generate go run golang.org/x/crypto/internal/polyval/internal/cmd/gen ctmul
...
func (p *Polyval) Init(key []byte) error {
	if len(key) != 16 {
		return fmt.Errorf("polyval: invalid key size: %d", len(key))
	}
	var zeroKey [16]byte
	if subtle.ConstantTimeCompare(key, zeroKey[:]) == 1 {
		return errors.New("polyval: the zero key is invalid")
	}
