...
func TestChaCha20Poly1305(t *testing.T) {
	...
	testAeadSealOpen := func(t *testing.T, aead cipher.AEAD, tv *AeadTestVector) {
		defer func() {
			if r := recover(); r != nil {
				// A bad nonce causes a panic in AEAD.Seal
				// and AEAD.Open, so should be recovered.
				if !hasBadNonce(tv.TcId) {
		...
		// Decrypt the provided ciphertext with a tag and validate the decrypted message.
		...
		if err != nil {
			return // don't validate message if decryption failed
		}
	...
	for _, tg := range root.TestGroups {
		for _, tv := range tg.Tests {
			...
			// IvSize/TagSize is in bits, but NonceSize/Overhead is in bytes, so divide by 8.
			if !hasBadNonce(tv.TcId) && tg.IvSize/8 != chacha20poly1305.NonceSize {
				t.Fatalf("#%d: bad nonce length: %d", tv.TcId, tg.IvSize)
			}
			if tg.TagSize/8 != aead.Overhead() {
				t.Fatalf("#%d: bad tag length", tv.TcId)
			}
			testAeadSealOpen(t, aead, tv)
		}
	}
}
