...
func TestEcdsa(t *testing.T) {
	...
	flagsShouldPass := map[string]bool{
		"EdgeCase":    true,
		"MissingZero": false, // An encoded integer missing a leading zero can lead to an invalid signature.
	...
	b := readTestVector("ecdsa_test.json")
	var root EcdsaRoot
	...
	for _, tg := range root.TestGroups {
		...
		pub := decodeKey(tg.KeyDer).(*ecdsa.PublicKey)
		h, _ := parseHash(tg.Sha)
		for _, sig := range tg.Tests {
			h.Reset()
			h.Write(decodeHex(sig.Msg))
			hashed := h.Sum(nil)
			result := wecdsa.VerifyASN1(pub, hashed, decodeHex(sig.Sig))
