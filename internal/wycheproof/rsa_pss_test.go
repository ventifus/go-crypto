...
func TestRsaPss(t *testing.T) {
	...
	flagsShouldPass := map[string]bool{
		// A signature using a weaker hash than the EC params is not a security risk.
		"WeakHash": true,
	}

	// passingZeroSaltLengthTests is the list of TcIds that
	// should explicitly pass in rsa_pss_2048_sha256_mgf1_0_test.json.
	...
	for _, f := range files {
		...
		for _, tg := range root.TestGroups {
			...
			for _, sig := range tg.Tests {
				...
				if (err == nil) != want {
					t.Errorf("file: %v, tcid: %d, type: %s, comment: %q, wanted success: %t", f, sig.TcId, sig.Result, sig.Comment, want)
				}
			}
		}
	}
