		...
		for _, tg := range root.TestGroups {
			...
			for _, tt := range tg.Tests {
				...
				t.Run(fmt.Sprintf("%s/%d", tg.Curve, tt.TcID), func(t *testing.T) {
					...
					if err != nil {
						if shouldPass {
							t.Errorf("unexpected pub parsing error: %s", err)
						}
						return
					}
					...
					if len(privBytes) != curveToKeySize[tg.Curve] {
						t.Skip("we don't support non-standard key sizes")
					}

					priv, err := curve.NewPrivateKey(privBytes)
					if err != nil {
						if shouldPass {
							t.Errorf("unexpected priv parsing error: %s", err)
						}
						return
					}
					...
					if err != nil {
						if shouldPass {
							t.Errorf("unexpected ECDH error: %s", err)
						}
						return
					}
					got := bytes.Equal(shared, x)
