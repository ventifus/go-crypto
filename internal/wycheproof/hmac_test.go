	...
	for f := range fileHashAlgs {
		...
		for _, tg := range root.TestGroups {
			for _, tv := range tg.Tests {
				h := parseHash(fileHashAlgs[f]).New
				hm := hmac.New(h, decodeHex(tv.Key))
				hm.Write(decodeHex(tv.Msg))
				tag := hm.Sum(nil)
				if tg.TagSize/8 != len(tag) {
					tag = tag[:tg.TagSize/8]
				}
				wantPass := shouldPass(tv.Result, tv.Flags, nil)
				match := bytes.Equal(decodeHex(tv.Tag), tag)
				if (wantPass && !match) || (!wantPass && match) {
