...
func TestMain(m *testing.M) {
	if runtime.GOOS == "js" {
		// go command not supported on js, so skip the tests.
		log.Print("skipping tests for js")
		os.Exit(0)
	}
	...
	// TODO: Try checking the module cache before downloading,
	// so it doesn't have to be re-fetched every time.
	...
	if err != nil {
		log.Fatalf("failed to run %q, output: %s", cmd, output)
	}
	var dm struct {
