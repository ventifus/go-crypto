...
func (c *Client) DeactivateReg(ctx context.Context) error {
	...
	res, err := c.post(ctx, nil /* use c.kid */, url, req, wantStatusOK)
	if err != nil {
		return err
	}
	res.Body.Close()
...
func (c *Client) getRegRFC(ctx context.Context) (*Account, error) {
	...
	res, err := c.post(ctx, c.Key, c.dir.RegURL, req, wantStatusOK)
...
func (c *Client) AuthorizeOrder(ctx context.Context, id []AuthzID, opt ...OrderOption) (*Order, error) {
	...
	res, err := c.post(ctx, nil /* use c.kid */, dir.OrderURL, req, wantStatusCreated)
	if err != nil {
		return nil, err
	}
	defer res.Body.Close()
...
func (c *Client) GetOrder(ctx context.Context, url string) (*Order, error) {
	...
	res, err := c.postAsGet(ctx, url, wantStatusOK)
	if err != nil {
		return nil, err
	}
	defer res.Body.Close()
...
// StatusReady, StatusValid or StatusInvalid, the CA responded with a 4xx error code,
...
func responseOrder(res *http.Response) (*Order, error) {
	...
	if err := json.NewDecoder(res.Body).Decode(&v); err != nil {
		return nil, fmt.Errorf("acme: invalid order response: %v", err)
	}
	o := &Order{
		URI:         res.Header.Get("Location"),
		Status:      v.Status,
