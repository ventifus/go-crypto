...
// TEST_HOST=xxx.ngrok.io:8080 ACME_DIRECTORY_URL=https://acme-staging-v02.api.letsencrypt.org/directory TEST_ACCOUNT_GET=1 go test
func TestIntegration(t *testing.T) {
	...
	// these endpoints are not supported by pebble, so put them behind a flag
	if testAccountGet {
		// test get with cached account URL
		c.AccountURL = na.URL
		...
		/* blocked on deployment of https://github.com/letsencrypt/boulder/issues/3327
		// clear cached account URL
		c = &acme.Client{Key: key, DirectoryURL: c.DirectoryURL, HTTPClient: c.HTTPClient}

		// test get without cached account URL
		na, err = c.GetAccount(context.Background())
		if err != nil {
			t.Fatal(err)
		}
	...
	order, err := c.CreateOrder(context.Background(), acme.NewOrderWithDomains(testIdentifier))
	if err != nil {
		t.Fatal(err)
	}
