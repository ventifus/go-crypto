...
type Manager struct {
	...
	// A better overall TLS SNI performance can be achieved with a non-nil Cache.
	//
	// If Cache is nil, all obtained certificates will be lost after Manager termination,
	// without a posibility of recovery.
	Cache Cache
	...
	// Contact is an optional slice of contact info used during account registration.
	// Each element is a URI. For instance, using an email address as the contact info
	// would take this form: []string{"mailto:user@example.org"}.
	Contact []string
	...
	certState   map[string]*certState // keyed by domain name
...
func (m *Manager) cacheGet(domain string) (*tls.Certificate, error) {
	...
	pubDER, privDER, err := m.Cache.Get(domain)
	if err != nil {
		return nil, err
	}
...
func (m *Manager) cachePut(domain string, tlscert *tls.Certificate) error {
	...
	return m.Cache.Put(domain, pub, prv)
}
...
// It will loop over m.Cache.Delete indefinitely, until the latter succeeds,
// and so deleteTokenCert must always be called from a separate goroutine.
...
func (m *Manager) acmeClient() (*acme.Client, error) {
	...
	a := &acme.Account{Contact: m.Contact}
