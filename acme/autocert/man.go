...
//
//	m := autocert.Manager{Prompt: AcceptTOS}
//	s := &http.Server{
//		Addr: ":https",
...
type Manager struct {
	...
	// It is typically an email address, for instance: []string{"mailto:user@example.org"}.
	Contact []string

	// The registration may require the caller to agree to the CA Terms of Service (TOS).
	// If so, Manager calls Prompt with a TOS URL provided by the CA. Prompt should report
	// whether the caller agrees to the terms. To always accept the terms,
	// the callers can use AcceptTOS.
...
func (m *Manager) cacheGet(domain string) (*tls.Certificate, error) {
	...
	if len(x509Cert) == 0 {
		return nil, errors.New("no public key found in cache")
	}
	leaf := x509Cert[0]
