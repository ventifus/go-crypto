...
type Manager struct {
	...
	//
	// If Client is nil, a zero-value acme.Client is used with the directory URL
	// pointing to Let's Encrypt RFC 8555 implementation.
	...
	// This is used by CAs to notify about problems with issued certificates.
	//
...
func (m *Manager) verify(ctx context.Context, client *acme.Client, domain string) error {
	...
	defer func() {
		// Use "detached" context. Authorization deactivations
		// need not happen in the current verification flow.
		go m.revokePendingAuthz(context.Background(), authzURLs)
	}()
...
// verify runs the identifier (domain) order-based authorization flow for RFC compliant CAs
// using each applicable ACME challenge type.
...
AuthorizeOrderLoop:
	for {
		...
		defer func() {
			// Use "detached" context. Authorization deactivations
			// need not happen in the current verification flow.
			go m.revokePendingAuthz(context.Background(), o.AuthzURLs)
		}()
