...
// ACME_ALPN_PROTO is the ALPN protocol name used by a CA server when validating
...
// See Manager.Config method for easy setup of net/http.Server servers.
const ACME_ALPN_PROTO = "acme-tls/1"
...
// supporting "HTTP/2" and ACME_ALPN_PROTO.
func (m *Manager) TLSConfig() *tls.Config {
	return &tls.Config{
		GetCertificate: m.GetCertificate,
		NextProtos: []string{
			"h2", "http/1.1", // enable HTTP/2
			ACME_ALPN_PROTO, // enable tls-alpn ACME challenges
		},
	}
}
...
func wantsTokenCert(hello *tls.ClientHelloInfo) bool {
	// tls-alpn-01
	if len(hello.SupportedProtos) == 1 && hello.SupportedProtos[0] == ACME_ALPN_PROTO {
		return true
	}
	// tls-sni-xx
