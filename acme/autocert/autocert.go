...
type Manager struct {
	...
	// renewal tracks the set of domains currently running renewal goroutines.
...
func (m *Manager) createCert(ctx context.Context, domain string) (*tls.Certificate, error) {
	state, exist, err := m.certState(domain)
	if err != nil {
		return nil, err
	}
...
// certState always returns a non-nil certState.
// Either an existing one is picked from m.state using domain as the map key,
// or a new one is created. In the latter case the new state is locked for read/write
// and added to m.state before returning.
// The returned error is non-nil only in the case where a new state could not be created.
func (m *Manager) certState(domain string) (state *certState, exist bool, err error) {
	m.stateMu.Lock()
	defer m.stateMu.Unlock()
	if m.state == nil {
		m.state = make(map[string]*certState)
...
func (m *Manager) renewLoop(domain string, key crypto.Signer, exp time.Time) {
	...
	f = func() {
		...
		time.AfterFunc(next, f)
	}
	time.AfterFunc(m.nextRenew(exp), f)
}
