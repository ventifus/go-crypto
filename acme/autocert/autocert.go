...
type Manager struct {
	...
	// If true, will try "http-01" challenge type in addition to tls-sni-xx
	// during the domain authorization flow.
	tryHTTP01 bool

	tokensMu sync.RWMutex // guards certTokens and httpTokens
...
// HTTPHandler configures the Manager to respond to ACME "http-01" challenges.
...
func (m *Manager) verify(ctx context.Context, domain string) error {
	...
	// Index of the next preferred challenge type.
	var nextTyp int
	for {
