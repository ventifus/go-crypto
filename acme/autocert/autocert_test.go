...
func TestNextRenew(t *testing.T) {
	now := time.Now()
	...
	for i, test := range tt {
		next := man.nextRenew(test.expiry)
...
func TestRenewFromCache(t *testing.T) {
	...
	// veriy renewal happened
	defer func() { testDidRenewLoop = nop }()
	done := make(chan struct{})
	testDidRenewLoop = func() {
		defer close(done)
		...
		if !tlscert.Leaf.NotAfter.After(future) {
			t.Errorf("cache leaf.NotAfter = %v; want > %v", tlscert.Leaf.NotAfter, future)
		}
		man.stateMu.Lock()
		...
		if !tlscert.Leaf.NotAfter.After(future) {
			t.Errorf("state leaf.NotAfter = %v; want > %v", tlscert.Leaf.NotAfter, future)
		}
	}
