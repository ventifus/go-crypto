...
func (m memCache) Get(ctx context.Context, name string) ([]byte, []byte, error) {
	v, ok := m[name]
	if !ok {
...
func (m memCache) Put(ctx context.Context, name string, pub, prv []byte) error {
...
func TestCache(t *testing.T) {
	...
	for i, test := range tt {
		cache.Put(nil, test.name, test.pub, test.prv)
