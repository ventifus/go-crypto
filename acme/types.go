...
type Error struct {
	...
	// ProblemType is a URI that identifies the problem type, typically in
...
func (e *Error) Error() string {
	return fmt.Sprintf("acme: %s: %s", e.ProblemType, e.Detail)
}

// An ErrorSubproblem is additional error detail that is included in an Error,
// usually indicating a problem with a specific identifier during authorization.
type ErrorSubproblem struct {
...
func (e *ErrorSubproblem) Error() string {
	if e.Identifier != nil {
		return fmt.Sprintf("acme: %s for %s: %s", e.Type, e.Identifier.Value, e.Detail)
	}
...
func (e OrderInvalidError) Error() string {
	...
	return fmt.Sprintf("acme: invalid order (%s): %s", e.Order.Error.ProblemType, e.Order.Error.Detail)
}
...
func RateLimit(err error) (time.Duration, bool) {
	e, ok := err.(*Error)
	if !ok || e.ProblemType != "urn:ietf:params:acme:error:rateLimited" {
		return 0, false
	}
	if e.Header == nil {
		return 0, true
...
// NewOrderWithDomains creates a new order with the DNS identifiers provided,
// suitable for use with CreateOrder.
func NewOrderWithDomains(domains ...string) *Order {
...
func (o *wireOrder) order(url string) *Order {
	return &Order{
		URL:            url,
		Status:         o.Status,
		Expires:        o.Expires,
