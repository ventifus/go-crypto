...
// WithPrivateKey creates an option holding a private/public key pair.
...
//
// The token argument is a Challenge.Token value.
// If a WithPrivateKey option is provided, its private part signs the returned cert,
// and the public part is used to specify the signee.
// If no WithPrivateKey option is provided, a new ECDSA key is generated using P-256 curve.
//
...
// To create a cert with a custom key pair, specify WithPrivateKey option.
func tlsChallengeCert(san []string, opt []CertOption) (tls.Certificate, error) {
	var key crypto.Signer
	for _, o := range opt {
		if o, ok := o.(*certOptKey); ok {
			key = o.key
			break
		}
	}
