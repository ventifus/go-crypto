...
//
// The reason argument specifies the revocation reason code as defined in RFC 5280.
// Callers can use OCSPReasonXxx constants provided in this package to indicate the code.
//
// The request may be cancelled with ctx.
func (c *Client) RevokeCert(ctx context.Context, key crypto.Signer, cert []byte, reason int) error {
	if _, err := c.Discover(ctx); err != nil {
		return err
	}
