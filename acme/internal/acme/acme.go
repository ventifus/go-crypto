...
// If the bundle argument is true, the returned value will also contain CA (the issuer) certificate chain.
//
// In the case where CA server does not provide the issued certificate in the response,
// CreateCert will poll certURL using c.FetchCert, which will result in additional round-trips.
// In such scenario the caller can cancel the polling with ctx.
//
// CreateCert returns an error if the CA's response, or its chain given the bundle is true, was unreasonably large.
...
//
// The returned value will also contain CA (the issuer) certificate if the bundle argument is true.
//
// FetchCert returns an error if the CA's response, or its chain given the bundle is true,
