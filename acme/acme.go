...
type Client struct {
	...
	// AccountURL is the URL of the account registered with the CA. It is
	// optional, and will be looked up using the Key if necessary.
	AccountURL string
	...
	urlMu      sync.Mutex // urlMu protects accountURL and ordersURL
	accountURL string
	ordersURL  string
}
...
func (c *Client) GetOrder(ctx context.Context, url string) (*Order, error) {
	return c.getOrder(ctx, url, false)
}
...
func (c *Client) getOrder(ctx context.Context, url string, waitFinal bool) (*Order, error) {
	sleep := sleeper(ctx)
	for {
		...
		if !waitFinal || o.Status == StatusValid || o.Status == StatusInvalid {
			return o, nil
		}
		...
		if err := sleep(res.Header.Get("Retry-After"), 0); err != nil {
			return nil, err
		}
	}
}
...
func (c *Client) GetAccount(ctx context.Context) (*Account, error) {
	...
	if c.AccountURL != "" {
		return c.doAccount(ctx, c.AccountURL, false, nil)
	}
...
func (c *Client) retryPostJWS(ctx context.Context, key crypto.Signer, accountURL, url string, body interface{}) (*http.Response, error) {
	...
	for {
		...
		if res.StatusCode >= 400 && res.StatusCode <= 599 {
			err := responseError(res)
			res.Body.Close()
			if ae, ok := err.(*Error); ok && ae.ProblemType == "urn:ietf:params:acme:error:badNonce" {
