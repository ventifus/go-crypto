...
const (
	// pebbleModVersion is the module version used for Pebble and Pebble's challenge test server.
	// It is ignored if `-pebble-local-dir` is provided.
	pebbleModVersion = "v2.7.0"
	// startingPort is the first port number used for binding interface addresses. Each call to
	// takeNextPort() will increment a port number starting at this value.
	startingPort = 5555
)

var (
	pebbleLocalDir = flag.String("pebble-local-dir", "", "Local Pebble to use, instead of fetching from source")
...
func TestTLSALPN01(t *testing.T) {
	t.Parallel()

	env := startPebbleEnvironment(t, nil)

	// Start a TLS-ALPN-01 challenge response server on the configured TLS
	// challenge port. We'll configure it with a challenge response certificate
	// later once we have challenge tokens in-hand.
	challServer := newChallTLSServer(fmt.Sprintf(":%d", env.config.TLSPort))
	challServer.Run()
	t.Cleanup(func() {
		challServer.Shutdown()
	})

	testIssuance(t, &env, challServer)
}

// challTLSServer is a simple challenge response server that listens for TLS connections
// on a specific port and if they are TLS-ALPN-01 challenge requests, completes the
// handshake using the configured challenge response certificate for the SNI value
// provided.
...
func (c *challTLSServer) Run() {
	go func() {
		// Note: certFile and keyFile empty because our config uses a GetCertificate callback.
...
func (c *challTLSServer) Provision(client *acme.Client, ident acme.AuthzID, chal *acme.Challenge) error {
	...
	if err != nil {
		return fmt.Errorf("challTLSServer: failed to generate challlenge response cert for %s: %w", ident.Value, err)
	}
...
func (c *challTLSServer) getCertificate(clientHello *tls.ClientHelloInfo) (*tls.Certificate, error) {
	...
	log.Printf("challTLSServer: selecting certificate for request from %s for %s", clientHello.Conn.RemoteAddr(), serverName)
...
func TestHTTP01(t *testing.T) {
	...
	// Start an HTTP-01 challenge response server on the configured HTTP
	// challenge port. We'll configure it with challenge response info
	// later when we have challenge tokens in-hand.
	challServer := newChallHTTPServer(fmt.Sprintf(":%d", env.config.HTTPPort))
	challServer.Run()
	t.Cleanup(func() {
		challServer.Shutdown()
	})
...
func (c *challHTTPServer) Provision(client *acme.Client, ident acme.AuthzID, chall *acme.Challenge) error {
	...
	if err != nil {
		return fmt.Errorf("failed to generate HTTP-01 challenge response for %v challenge %s token %s: %w", ident, chall.URI, chall.Token, err)
	}
...
func testIssuance(t *testing.T, env *environment, provisioner challengeProvisioner) {
	t.Helper()
	...
	// For each pending authz provision a challenge response and tell the server to verify it.
	for _, authzURL := range order.AuthzURLs {
		authz, err := client.GetAuthorization(ctx, authzURL)
		if err != nil {
			t.Fatalf("failed to get order %s authorization %s: %v", orderURL, authzURL, err)
		}
		...
		for _, challenge := range authz.Challenges {
			if challenge.Status != acme.StatusPending || !provisioner.Supported(challenge) {
				continue
			}

			if err := provisioner.Provision(client, authz.Identifier, challenge); err != nil {
				t.Fatalf("failed to provision challenge %s: %v", challenge.URI, err)
			}

			_, err = client.Accept(ctx, challenge)
			if err != nil {
				t.Fatalf("failed to accept order %s challenge %s: %v", orderURL, challenge.URI, err)
			}
		}
	}
	...
	if err != nil {
		t.Fatalf("failed to wait for order %s: %v", orderURL, err)
	}
	if order.Status != acme.StatusReady {
		t.Fatalf("expected order %s status to be ready, got %v", orderURL, order.Status)
	}

	// Generate a certificate keypair and a CSR using that keypair.
	...
	if err != nil {
		t.Fatalf("failed to finalize order %s with finalize URL %s: %v", orderURL, order.FinalizeURL, err)
	}
	...
	// Verify the chain using Pebble's issuing roots and the provided intermediates.
	...
	// Also verify that the leaf cert is valid for each of the DNS names and IP addresses
	// from our order's identifiers.
	for _, name := range dnsNames {
		if err := leafCert.VerifyHostname(name); err != nil {
			t.Fatalf("failed to verify order %s leaf certificate for order DNS name %s: %v", orderURL, name, err)
		}
	}
	for _, ip := range ipAddresses {
		if err := leafCert.VerifyHostname(ip.String()); err != nil {
			t.Fatalf("failed to verify order %s leaf certificate for order IP address %s: %v", orderURL, ip, err)
		}
	}
}

// challengeProvisioner is a challenge-type agnostic interface for provisioning
// a challenge response for a specific identifier's pending challenge.
type challengeProvisioner interface {
...
// RootCert returns the Pebble CA's issuing hierarchy root certificate.
// It is generated randomly at each startup and can be used to verify
...
func (e *environment) RootCert() (*x509.CertPool, error) {
	// NOTE: in the future we may want to consider the alternative chains returned as
	//       Link alternative headers.
...
// startPebbleEnvironment is a test helper that spawns Pebble and Pebble challenge test server
// processes based on the provided environmentConfig. The processes will be torn down when the
// test ends.
//
// This function will skip the current test if short mode is enabled or if the runner isn't Linux.
func startPebbleEnvironment(t *testing.T, config *environmentConfig) environment {
	...
	// Spawn the Pebble CA server. It answers ACME requests and performs outbound validations.
	// We configure it to use a mock DNS server that always answers 127.0.0.1 for all A/AAAA
	// queries.
	spawnServerProcess(t, pebbleDir, "./cmd/pebble", "-config", configFile.Name(),
		"-dnsserver", fmt.Sprintf("127.0.0.1:%d", config.dnsPort),
		"-strict")

	// Spawn the Pebble challenge test server. We'll use it to mock DNS responses but disable all
	// the other interfaces. We want to stand up our own challenge response servers for TLS-ALPN-01,
	// etc.
	spawnServerProcess(t, pebbleDir, "./cmd/pebble-challtestsrv",
	...
	// Create an ACME account keypair/client and verify it can discover the directory
	// without error.
