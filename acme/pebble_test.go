...
func TestWithPebble(t *testing.T) {
	...
	tests := []struct {
		name     string
		challSrv func(*environment) challengeServer
	}{
		{
			name: "TLSALPN01-Issuance",
			challSrv: func(env *environment) challengeServer {
				return newChallTLSServer(fmt.Sprintf(":%d", env.config.TLSPort))
			},
		},
		{
			name: "HTTP01-Issuance",
			challSrv: func(env *environment) challengeServer {
				return newChallHTTPServer(fmt.Sprintf(":%d", env.config.HTTPPort))
			},
		},
	}
...
func startPebbleEnvironment(t *testing.T, config *environmentConfig) environment {
	...
	// always answers 127.0.0.1 for all A/AAAA queries so that validation
