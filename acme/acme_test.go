...
func TestNonce_add(t *testing.T) {
	client := &Client{nonces: make([]string, maxNonces-1)}
	for i := range client.nonces {
		client.nonces[i] = fmt.Sprintf("%d", i)
	}
	tt := []struct {
		header http.Header
		first3 []string
		...
		{http.Header{"Replay-Nonce": {"foo"}}, []string{"0", "1", "2"}, []string{"97", "98", "foo"}},
		{http.Header{"Replay-Nonce": {"bar"}}, []string{"1", "2", "3"}, []string{"98", "foo", "bar"}},
		{http.Header{"Replay-Nonce": {"baz"}}, []string{"2", "3", "4"}, []string{"foo", "bar", "baz"}},
	}
	for i, test := range tt {
		client.addNonce(test.header)
		first3 := client.nonces[:3]
		if !reflect.DeepEqual(first3, test.first3) {
			t.Errorf("%d: first3 = %q; want %q", i, first3, test.first3)
		}
		last3 := client.nonces[len(client.nonces)-3:]
		if !reflect.DeepEqual(last3, test.last3) {
			t.Errorf("%d: last3 = %q; want %q", i, last3, test.last3)
		}
		if n := len(client.nonces); n > maxNonces {
			t.Errorf("%d: len(client.nonces) = %d; want <= %d", i, n, maxNonces)
		}
	}
}

func TestNonce_addUnique(t *testing.T) {
	h := http.Header{"Replay-Nonce": {"nonce"}}
	var c Client
	c.addNonce(h)
	c.addNonce(h)
...
func TestNonce_pop(t *testing.T) {
	var count int
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	client := &Client{nonces: []string{"first"}}
	expected := []string{"first", "nonce0", "nonce1"}
	for i, nonce := range expected {
		res, err := client.popNonce(context.Background(), ts.URL)
		if err != nil {
			t.Errorf("%d: popNonce: %v", i, err)
			continue
		}
		if res != nonce {
			t.Errorf("%d: popNonce = %q; want %q", i, res, nonce)
		}
	}
	if len(client.nonces) != 0 {
		t.Errorf("client.nonces = %q; want []", client.nonces)
	}
}
...
func TestNonce_postJWS(t *testing.T) {
	var count int
	// Nonce values generated by the ts in replay-nonce header,
	// keyed by the count value.
	nonces := map[int]string{
		2: "nonce1", // second request
		3: "nonce2", // third request
	}
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		count++
		w.Header().Set("replay-nonce", fmt.Sprintf("nonce%d", count))
		...
		// Verify that the request includes the nonce we expect.
		head, err := decodeJWSHead(r)
		if err != nil {
			t.Errorf("decodeJWSHead: %v", err)
			return
		}
		if head.Nonce != nonces[count] {
			t.Errorf("head.Nonce = %q; want %q", head.Nonce, nonces[count])
		}
	}))
	defer ts.Close()
	...
	// The second call should not generate another HEAD /directory request.
	if _, err := client.Authorize(context.Background(), "example.com"); err != nil {
		t.Errorf("client.Authorize 2: %v", err)
	}

	last := []string{"nonce3"} // last nonce collected from client.Authorize
	if !reflect.DeepEqual(client.nonces, last) {
		t.Errorf("client.nonces = %q; want %q", client.nonces, last)
	}
	if count != 3 {
