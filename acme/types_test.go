	...
	tests := []struct {
		...
		},
		{
			...
			expectedOut: Error{
				...
				ProblemType: "urn:error",
				Detail:      "it's an error",
				Subproblems: []SubError{
					{
						ProblemType: "urn:error:sub",
						Detail:      "it's a subproblem",
					},
				},
			},
...
func TestErrorStringerWithSubproblems(t *testing.T) {
	...
	expectedStr := "1 urn:error: it's an error. This error contains the following subproblems: [urn:error:sub: it's a subproblem, urn:error:sub: [dns: example] it's a subproblem]"
